...
def build_conditioning_prefix(
    ...
    item_processor: Optional[FlexARItemProcessor],
    prompt: Optional[str],
    conditioning_images: Optional[List[Union[str, Image.Image]]],
) -> torch.Tensor:
    # Construct prefix tokens (including BOS) that encode additional conditional context.
    if item_processor is None or (prompt is None and not conditioning_images):
        return torch.tensor([vocab_info.begin_sequence], dtype=torch.int64)

    conversations: List[Dict[str, Optional[str]]] = []
    conversations.append({"from": "human", "value": prompt or ""})
    conversations.append({"from": "gpt", "value": None})

    item = {
        "image": conditioning_images or [],
        "conversations": conversations,
    }

    raw_tokens = item_processor.process_item(item)
    prefix_tokens: List[int] = []
    for value in raw_tokens:
        if isinstance(value, int):
            prefix_tokens.append(int(value))
        else:
            prefix_tokens.extend(int(tok) for tok in value["input_ids"])

    if not prefix_tokens:
        prefix_tokens = [vocab_info.begin_sequence]

    if prefix_tokens[0] != vocab_info.begin_sequence:
        prefix_tokens = [vocab_info.begin_sequence] + prefix_tokens

    if prefix_tokens[-1] == vocab_info.end_sequence:
        prefix_tokens = prefix_tokens[:-1] or [vocab_info.begin_sequence]

    return torch.tensor(prefix_tokens, dtype=torch.int64)
...
def estimate_entropy_bits(
    ...
    prefix = new_bpe_tok_seq[0:prefix_len]

    torch_device = torch.device(device)
    device_type = torch_device.type
    model_dtype = next(model.parameters()).dtype
    use_autocast = device_type == "cuda" and model_dtype in (torch.float16, torch.bfloat16)

    with torch.inference_mode():
        for idx in trange(prefix_len, bpe_tok_seq.numel() - 1, ncols=80):  # ignore the last token (EOS)
            input_ids = prefix.unsqueeze(0).to(torch_device, non_blocking=True)
            autocast_cm = (
            ...
            logits = logits[4 : 8195 + 1]
            logits_cpu = logits.to(dtype=torch.float32, device="cpu")
            log_probs = torch.log_softmax(logits_cpu, dim=-1)
            probs = torch.exp(log_probs)
            dist_entropy_bits = float(-(probs * log_probs).sum().item() / math.log(2))
            distribution_bits.append(dist_entropy_bits)

            if torch.rand(1).item() < gen_probabilities:
                bpe_tok_offset = int(torch.argmax(logits_cpu).item()) + 4
                new_bpe_tok_seq[idx] = bpe_tok_offset
                bits = 0.0
            else:
                bpe_tok = int(new_bpe_tok_seq[idx].item())

                if bpe_cluster_lookup and bpe_tok in bpe_cluster_lookup:
                    members = bpe_cluster_lookup[bpe_tok]
                    cluster_log_prob = torch.logsumexp(log_probs[members], dim=0)
                    bits = float(-cluster_log_prob.item() / math.log(2))
                else:
                    token_index = bpe_tok - 4
                    if token_index < 0 or token_index >= log_probs.numel():
                        raise IndexError(
                            f"Token index {token_index} out of bounds for logits of size {log_probs.numel()}"
                        )
                    token_log_prob = log_probs[token_index].item()
...
def analyze_entropy(
    ...
    if prefix_tokens is not None and prefix_tokens.numel() > 0:
        prefix_tokens_cpu = prefix_tokens.to(dtype=torch.int64, device=sequence.device)
        if prefix_tokens_cpu[0].item() != sequence[0].item():
            raise ValueError("Prefix tokens must start with the same BOS token as the image sequence")
        combined_sequence = torch.cat([prefix_tokens_cpu, sequence[1:]])
        prefix_len = int(prefix_tokens_cpu.numel())
    else:
        combined_sequence = sequence
        prefix_len = 1

    num_tokens = combined_sequence.numel() - prefix_len
    ...
    effective_tokens = max(num_tokens - 1, 1)
    print(f"Tokens:       {max(num_tokens-1, 0)}")
    print(f"Entropy bits: {total_bits:.4f}")
    print(f"Average per-token bits: {total_bits/effective_tokens:.4f}")
    print(f"Entropy bpp:  {bpp:.6f}")

    csv_path = os.path.join(log_path, "per_entropy.csv")
    png_path = os.path.join(log_path, "entropy.png")

    target_tokens = gen_seq[prefix_len : prefix_len + num_tokens].cpu().numpy() # target tokens include EOS
    img_token_ids = [
    ...
    return total_bits, bpp, gen_seq[prefix_len : -1]  # exclude prefix and EOS
...
def process_image(
    ...
    *,
    ...
    decode_bpe_tokens = gen_bpe_tokens.to(torch.int64).cpu()
...
def main():
    ...
    parser.add_argument("--prompt", type=str, default=None, help="Optional text prompt for conditional entropy estimation")
    parser.add_argument("--prompt-file", type=str, default=None, help="Path to a text file containing the conditional prompt")
    parser.add_argument("--cond-image", action="append", default=None, help="Additional conditioning image path (repeatable)")
    parser.add_argument("--target-size", type=int, default=512, help="Target resolution used by the conditioning item processor")
    ...
    conditioning_images: List[Union[str, Image.Image]]
    conditioning_images = [] if args.cond_image is None else args.cond_image

    image_tokenizer, vocab_info, vocab_translation = init_tokenizer(args.device)

    item_processor: Optional[FlexARItemProcessor] = None
    if prompt_text is not None or conditioning_images:
        item_processor = FlexARItemProcessor(target_size=args.target_size)
    ...
    prefix_tokens = build_conditioning_prefix(
        vocab_info=vocab_info,
